...
create table if not exists public.rating_cheers (
...
create index if not exists rating_cheers_rating_idx
  on public.rating_cheers(rating_id, created_at);

alter table public.rating_cheers enable row level security;

drop policy if exists rating_cheers_read_club on public.rating_cheers;
create policy rating_cheers_read_club on public.rating_cheers
for select to authenticated
using (
  exists (
    select 1
    from public.ratings r
    where r.id = rating_cheers.rating_id
      and public.is_club_member(r.club_id)
  )
);

drop policy if exists rating_cheers_insert_self on public.rating_cheers;
create policy rating_cheers_insert_self on public.rating_cheers
...
drop policy if exists rating_cheers_delete_self on public.rating_cheers;
create policy rating_cheers_delete_self on public.rating_cheers
for delete to authenticated
using (user_id = auth.uid());

grant select, insert, delete on public.rating_cheers to authenticated;
