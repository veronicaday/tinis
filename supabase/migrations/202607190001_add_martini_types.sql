...
alter table public.ratings
  add column if not exists martini_type text not null default 'classic';

alter table public.ratings
  drop constraint if exists ratings_martini_type_check;
alter table public.ratings
  add constraint ratings_martini_type_check
  check (martini_type in ('classic', 'espresso'));

alter table public.ratings
  drop constraint if exists ratings_spirit_check;
alter table public.ratings
  add constraint ratings_spirit_check
  check (spirit is null or spirit in (
    'gin', 'vodka', 'both', 'unknown', 'tequila', 'rum', 'other'
  ));

alter table public.personal_venue_ranks
  add column if not exists martini_type text not null default 'classic';
alter table public.personal_venue_ranks
  drop constraint if exists personal_venue_ranks_martini_type_check;
alter table public.personal_venue_ranks
  add constraint personal_venue_ranks_martini_type_check
  check (martini_type in ('classic', 'espresso'));
alter table public.personal_venue_ranks
  drop constraint if exists personal_venue_ranks_pkey;
alter table public.personal_venue_ranks
  add primary key (user_id, venue_id, martini_type);

alter table public.comparisons
  add column if not exists martini_type text not null default 'classic';
alter table public.comparisons
  drop constraint if exists comparisons_martini_type_check;
alter table public.comparisons
  add constraint comparisons_martini_type_check
  check (martini_type in ('classic', 'espresso'));

create index if not exists ratings_user_venue_type_visited_idx
  on public.ratings(user_id, venue_id, martini_type, visited_at desc);
...
create function public.save_rating(
  ...
  p_martini_type text default 'classic'
)
returns uuid
language plpgsql
security definer
...
declare
  ...
  normalized_martini_type text := lower(coalesce(nullif(trim(p_martini_type), ''), 'classic'));
begin
  ...
  if normalized_martini_type not in ('classic', 'espresso') then
    raise exception 'Unsupported martini type';
  end if;
  ...
  insert into public.ratings (
    ...
    normalized_martini_type
  ) returning id into new_rating_id;

  insert into public.personal_venue_ranks (user_id, venue_id, martini_type)
  values (auth.uid(), target_venue_id, normalized_martini_type)
  on conflict (user_id, venue_id, martini_type) do nothing;
...
revoke all on function public.save_rating(
  uuid, text, text, numeric, numeric, numeric,
  numeric, numeric, text, text, text, numeric, text, text, text
) from public, anon;
...
drop view if exists public.club_leaderboard;

create view public.friend_feed with (security_invoker = true) as
select
  r.id, r.club_id, r.user_id, p.display_name, p.avatar_path,
  r.venue_id, v.name as venue_name, r.martini_type, v.city, v.region, r.score,
...
create view public.club_leaderboard with (security_invoker = true) as
with latest_per_member as (
  select distinct on (r.club_id, r.user_id, r.venue_id, r.martini_type)
    r.id as rating_id, r.club_id, r.user_id, r.venue_id, r.martini_type,
    r.score, r.dirtiness, r.chilliness, r.uniqueness, r.spirit_forward,
    r.spirit, r.garnish, r.serving_style, r.price, r.public_note, r.visited_at
  from public.ratings r
  order by r.club_id, r.user_id, r.venue_id, r.martini_type,
    r.visited_at desc, r.created_at desc
),
club_average as (
  select club_id, martini_type, avg(score)::numeric as score
  from latest_per_member
  group by club_id, martini_type
),
venue_scores as (
  select l.club_id, l.venue_id, l.martini_type, count(*)::integer as rating_count,
  ...
  from latest_per_member l
  group by l.club_id, l.venue_id, l.martini_type
)
select s.club_id, s.venue_id, v.name as venue_name, s.martini_type, v.city, v.region,
  ...
  nullif(v.address, '') as full_address
from venue_scores s
join club_average a using (club_id, martini_type)
